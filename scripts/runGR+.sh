...
intersectBed -a ${cage}+ -b ${gpe}.firstexon+.1000 -wa -wb |awk '{if ($4==$8)print $0}'|awk '{print $1"\t"$2"\t"$6+1000"\tcage:"$4":"$2"-"$3"\t"$0}'|cut -f 1-4,13- > ${gpe}.RawDistance.cage.trans+.gene
...
less -S ${gpe}.level4.noJunction.transGene.cage+ |sed 's/,/\t/g'|sed 's/cage:+://g'|sed 's/-/\t/g'|awk '{a=2; for (i=3;i<=NF;i++) {if($i>$a)a=i };print $1"\tcage:+:"$a-1"-"$a  }'|sed 's/###/\t/g' > ${gpe}.nojunction.trans.gene.level4.cage.+ 
