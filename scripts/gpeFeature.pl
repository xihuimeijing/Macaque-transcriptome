...
my ($bin, $tss, $tts, $intron, $exon, $cds, $utr, $prime, $cmpl, $upstream, $dnstream, $chrSizeFile, $chain);
GetOptions(
            'b|bin'         => \$bin,
            'tss'           => \$tss,
            'tts'           => \$tts,
...
while(<GPE>){
    ...
    my ($name, $chr, $strand, $start, $end, $cdsStart, $cdsEnd, $exonStarts, $exonEnds, $cdsStartStat, $cdsEndStat) = @fields[0..6, 8, 9, 12, 13];
    ...
    if(defined $tss){
        ...
        if(defined $chain){
            ...
            for (my $i = 0; $i < @$intronStarts; $i++){
                ...
                my $intronID = "$name." . ($strand eq '+' ? $i+1 : @$intronStarts - $i);
                say join "\t", ($chr,
                                $intronStart,
                                $intronEnd,
                                $intronID,
        ...
        if(defined $chain){
            ...
            for (my $i = 0; $i < @exonStarts; $i++){
                my $exonID = "$name." . ($strand eq '+' ? $i+1 : @exonStarts - $i);
                say join "\t", ($chr,
                                $exonStarts[$i],
                                $exonEnds[$i],
                                $exonID,
        ...
        if(defined $chain){
            ...
            for (my $i = 0; $i < @$cdsStarts; $i++){
                my $cdsID = "$name." . ($strand eq '+' ? $i+1 : @$cdsStarts - $i);
                say join "\t", ($chr,
                                $cdsStarts->[$i],
                                $cdsEnds->[$i],
                                $cdsID,
        ...
        if(defined $prime){
            ...
            if(defined $chain){
                ...
                for (my $i = 0; $i < @$utrStarts; $i++){
                    my $utrID = "$name." . ($strand eq '+' ? $i+1 : @$utrStarts - $i);
                    say join "\t", ($chr,
                                    $utrStarts->[$i],
                                    $utrEnds->[$i],
                                    $utrID,
            ...
            if($cdsStart != $start){
                ...
                if(defined $chain){
                    ...
                    for (my $i = 0; $i < @$utrStarts; $i++){
                        my $utrID = "$name.L." . ($strand eq '+' ? $i+1 : @$utrStarts - $i);
                        say join "\t", ($chr,
                                        $utrStarts->[$i],
                                        $utrEnds->[$i],
                                        $utrID,
            ...
            if($cdsEnd != $end){
                ...
                if(defined $chain){
                    ...
                    for (my $i = 0; $i < @$utrStarts; $i++){
                        my $utrID = "$name.R." . ($strand eq '+' ? $i+1 : @$utrStarts - $i);
                        say join "\t", ($chr,
                                        $utrStarts->[$i],
                                        $utrEnds->[$i],
                                        $utrID,
