...
less -S ${gpe}.level2.noJunction.transGene.cage- |sed 's/,/\t/g'|sed 's/cage:-://g'|sed 's/-/\t/g'|awk '{a=2; for (i=3;i<=NF;i++) {if($i<$a)a=i };print $1"\tcage:-:"$a"-"$a+1  }'|sed 's/###/\t/g' > ${gpe}.nojunction.trans.gene.level2.cage.- 
