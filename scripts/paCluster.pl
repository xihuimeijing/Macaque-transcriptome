...
Usage: perl $scriptName consensus.bed12 >paCluster.bed8+
    If consensus.bed12 isn't specified, input from STDIN
    ...
    Relative positions (comma-separated) of cleavage sites in a cluster and their
        total number are put into 9th and 10th columns, respectively.
    The fraction of supporting reads at the mode cleavage region (in a window, see
        --windowSize) is put into 11th and those at each cleavage sites are appended as additional columns.
Options:
